...
function App() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col justify-between selection:bg-brand-primary/30 selection:text-white">
      ...
      {/* Persistent Navigation (Hidden on Admin panel) */}
      {!isAdminRoute && <Navbar />}

      {/* Main Content Area with Page Animations */}
      <main className={cn("flex-grow", !isAdminRoute && "pb-16")}>
        {isAdminRoute ? (
      ...
      {/* Premium Footer (Hidden on Admin panel) */}
      {!isAdminRoute && <Footer />}

      {/* Floating WhatsApp Button (Hidden on Admin panel) */}
      {!isAdminRoute && (
